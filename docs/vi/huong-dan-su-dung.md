...
5. Bấm biểu tượng loa trên node để nghe TTS khi provider API đã cấu hình.
...
Khi provider API offline, drawer báo offline; thư viện, mindmap, quiz và SRS vẫn hoạt động.
...
4. Bấm dừng.
5. Ứng dụng gửi đoạn ghi âm tới STT qua provider API.
...
App không chấm chất lượng phát âm. STT/TTS cần provider API; transcript thủ công và diff nội dung vẫn dùng được khi speech provider offline.
...
7. Bấm **Phân tích chương** để tạo bản nháp AI khi provider API online.
...
- Kiểm tra provider API đang chạy.
- Kiểm tra `PROVIDER_API_URL`.
- Kiểm tra model ID trong `.env`.
- Mở **Cài đặt → Kiểm tra kết nối**.

### Không nghe được TTS

- Cấu hình `PROVIDER_API_TTS_MODEL` hoặc `PROVIDER_API_TTS_VOICE`.
