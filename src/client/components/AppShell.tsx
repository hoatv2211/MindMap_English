import { BookOpen, BrainCircuit, ChartNoAxesCombined, Library, Menu, Settings, Sparkles, X } from "lucide-react";
import type { PropsWithChildren } from "react";
import { useAppStore, type Page } from "../state/app-store";

const nav:Array<{page:Page;label:string;icon:typeof BookOpen}>=[
  {page:"today",label:"Hôm nay",icon:BookOpen},{page:"library",label:"Thư viện",icon:Library},{page:"create",label:"Tạo mindmap",icon:BrainCircuit},{page:"progress",label:"Tiến độ",icon:ChartNoAxesCombined},
];
export function AppShell({children}:PropsWithChildren){
  ...
  return <div className={`app-shell ${focusMode?"is-focus":""}`}>
    ...
    <button className="mobile-menu" onClick={()=>setPage("today")}><Menu size={20}/></button>
  </div>
}
