...
export function MindmapCanvas({map,onNodeMove,onSpeak}:{map:Mindmap;onNodeMove:(nodeId:number,x:number,y:number)=>void;onSpeak:(text:string)=>void}){
  const nodes=useMemo<Node<VocabularyNodeData>[]>(()=>map.nodes.map(node=>({id:String(node.id),type:"vocabulary",position:{x:node.x,y:node.y},data:{label:node.label,meaningVi:node.meaningVi,ipa:node.ipa,nodeType:node.nodeType,color:node.color,status:node.status,onSpeak}})),[map,onSpeak]);
  ...
  return <div className="mindmap-canvas"><ReactFlow nodes={nodes} edges={edges} nodeTypes={{vocabulary:VocabularyNode}} fitView fitViewOptions={{padding:.24}} minZoom={.3} maxZoom={1.6} onNodeDragStop={(_,node)=>onNodeMove(Number(node.id),node.position.x,node.position.y)}><Background variant={BackgroundVariant.Dots} gap={22} size={1}/><Controls showInteractive={false}/><MiniMap pannable zoomable nodeColor={node=>`var(--branch-${String((node.data as VocabularyNodeData).color)})`}/></ReactFlow></div>
}
