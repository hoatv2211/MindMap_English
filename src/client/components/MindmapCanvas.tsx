...
export function MindmapCanvas({map,onNodeMove,onSpeak,onAddNote,onSelectNode}:{map:Mindmap;onNodeMove:(nodeId:number,x:number,y:number)=>void;onSpeak:(text:string)=>void;onAddNote?:(nodeId:number)=>void;onSelectNode?:(nodeId:number)=>void}){
 const nodes=useMemo<Node<VocabularyNodeData>[]>(()=>map.nodes.map(node=>({id:String(node.id),type:"vocabulary",position:{x:node.x,y:node.y},data:{nodeId:node.id,label:node.label,meaningVi:node.meaningVi,ipa:node.ipa,nodeType:node.nodeType,color:node.color,status:node.status,imageUrl:node.imageUrl,onSpeak,onAddNote}})),[map,onSpeak,onAddNote]);
