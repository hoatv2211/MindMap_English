...
export function createAuthRouter(service: AuthService, secureCookies: boolean, absoluteSessionHours=168) {
  const router = Router();
  const cookie = (response: import("express").Response, token: string) => response.cookie(SESSION_COOKIE, token, { httpOnly: true, sameSite: "lax", secure: secureCookies, path: "/", maxAge: absoluteSessionHours * 60 * 60 * 1000 });
