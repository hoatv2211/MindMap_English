...
export function createAgentRouter(service: AgentToolService) {
  ...
  router.post("/mindmap-drafts", async (request, response, next) => { try { response.status(201).json(await service.generateMindmapDraft(request.body)); } catch (error) { next(error); } });
  router.post("/mindmap-drafts/save", (request, response) => { const input = z.object({ topicId: z.number().int().positive(), draft: GeneratedMindmapSchema }).parse(request.body); response.status(201).json(service.saveGeneratedDraft(input.topicId, input.draft)); });
  return router;
}
