...
export class DocumentRepository {
  ...
  create(input: { title: string; originalFilename: string; format: "txt" | "md" | "epub"; mimeType: string; buffer: Buffer }) {
    const checksum = createHash("sha256").update(input.buffer).digest("hex");
    if (this.db.prepare("SELECT id FROM document_sources WHERE checksum=?").get(checksum)) return null;
    ...
    try {
      return withTransaction(this.db, () => {
        const id = Number(this.db.prepare(`INSERT INTO document_sources(title,original_filename,storage_path,format,mime_type,checksum,size_bytes)
          VALUES (?,?,?,?,?,?,?)`).run(input.title, input.originalFilename, relativePath, input.format, input.mimeType, checksum, input.buffer.length).lastInsertRowid);
  ...
  get(id: number) {
    const document = this.db.prepare(`SELECT id,title,original_filename originalFilename,storage_path storagePath,format,mime_type mimeType,checksum,size_bytes sizeBytes,created_at createdAt
      FROM document_sources WHERE id=?`).get(id) as Record<string, unknown> | undefined;
  ...
  addHighlight(documentId: number, input: { sectionId: number; selectedText: string; startOffset: number; endOffset: number; vocabularyId?: number | null; sentenceId?: number | null }) {
    ...
    const id = Number(this.db.prepare(`INSERT INTO document_highlights(document_id,section_id,vocabulary_id,sentence_id,selected_text,start_offset,end_offset,source_type,text_fingerprint)
      VALUES (?,?,?,?,?,?,?,'quoted',?)`).run(documentId, section.id, input.vocabularyId ?? null, input.sentenceId ?? null, input.selectedText, input.startOffset, input.endOffset, textFingerprint).lastInsertRowid);
  ...
  getSectionText(documentId: number, sectionIds: number[]): string | null {
