...
export function createDocumentRouter(repository: DocumentRepository, agent: AgentToolService) {
  const router = Router();
  router.get("/", (request, response) => response.json(repository.list((request as AuthenticatedRequest).auth?.id)));
  router.post("/", upload.single("document"), (request, response) => {
    ...
    const document = repository.create({ title, originalFilename: path.basename(request.file.originalname), format: rule.format, mimeType: request.file.mimetype, buffer: request.file.buffer }, (request as AuthenticatedRequest).auth?.id);
    return document ? response.status(201).json(document) : response.status(409).json({ error: "Document already exists" });
  });
  router.get("/:id", (request, response) => {
    const document = repository.get(Number(request.params.id), (request as AuthenticatedRequest).auth?.id);
  ...
  router.post("/:id/highlights", (request, response) => {
    ...
    const highlight = repository.addHighlight(Number(request.params.id), input, (request as AuthenticatedRequest).auth?.id);
